...
`mcp-metacritic` is a read-only client for [Metacritic](https://www.metacritic.com). It runs on your
...
becomes of them. `MC_LOG_LEVEL` governs how much is written and defaults to `error`. These lines stay on your machine.
...
Open an issue on [the repository](https://github.com/smeet666/mcp-metacritic/issues). For something exploitable,
...
`mcp-metacritic` est un client en lecture seule pour [Metacritic](https://www.metacritic.com). Il
...
qu'ils deviennent. `MC_LOG_LEVEL` règle leur quantité et vaut `error` par défaut. Ces lignes restent sur votre machine.
...
Ouvrez une issue sur [le dépôt](https://github.com/smeet666/mcp-metacritic/issues). Pour quelque chose
